...
def get_mnist_dataloaders(batch_size=128):
    ...
    train_data = datasets.MNIST(
        "../data", train=True, download=True, transform=all_transforms
    )
...
def get_fashion_mnist_dataloaders(batch_size=128):
    ...
    train_data = datasets.FashionMNIST(
        "../fashion_data", train=True, download=True, transform=all_transforms
    )
    test_data = datasets.FashionMNIST(
        "../fashion_data", train=False, transform=all_transforms
    )
    # Create dataloaders
    train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True)
    test_loader = DataLoader(test_data, batch_size=batch_size, shuffle=True)
...
def get_lsun_dataloader(path_to_data="../lsun", dataset="bedroom_train", batch_size=64):
    ...
    # Get dataset
    lsun_dset = datasets.LSUN(
        db_path=path_to_data, classes=[dataset], transform=transform
    )
