...
dataset = torch.load("./Experiment_1/generated_dataset.pt")
...
with open("results.txt", "w") as file:
    average_fom = sum(FOM_measurements) / len(FOM_measurements)
    max_fom = max(FOM_measurements)
    file.write(f"Average FOM vgg: {average_fom}")
    file.write(f"Max FOM vgg: {max_fom}")
