...
class VAE_GAN(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        total_loss = perceptual_loss_value + adv_loss
...
class VAE_GAN(pl.LightningModule):
    ...
    def __init__(self, a, b, c, d, in_channels = 1, h_dim = 32, lr = 1e-3, patch_dim = 32, image_dim = 64, batch_size = 100):
        ...
        self.QUBO_matrix = torch.randn((64, 64))
    ...
    def training_step(self, batch, batch_idx):
        ...
        energy = self.quboEnergy(binary_vector, self.QUBO_matrix)
        normalized_energy = F.sigmoid(energy)
        normalized_FOM = torch.divide((FOM_labels - self.FOM_min), (self.FOM_max - self.FOM_min))
        energy_loss = torch.exp(((1 - normalized_FOM) - normalized_energy), 2)

        total_loss = perceptual_loss_value + adv_loss + reconstruction_loss + energy_loss
        self.log("reconstruction_loss", reconstruction_loss)
        self.log("perceptual_loss", perceptual_loss_value)
...
class VAE(pl.LightningModule):
    ...
    def __init__(self, in_channels = 1, h_dim = 32, lr = 1e-3, batch_size = 100):
        ...
        self.decoder = nn.Sequential(nn.Linear(64, 64),
                                     nn.ReLU(),
                                     nn.Unflatten(1, (self.batch_size, 1, 8, 8)),
...
class GumbelSoftmax(pl.LightningModule):
    ...
    def forward(self, logits):
        ...
        z = torch.zeros_like(y, device = self.device)
        indices = torch.argmax(y, dim = 1)
        z.scatter_(1, indices.unsqueeze(1), 1)
